...
<security:authorize access="hasRole('DANCER')">
	...
	<jstl:if test="${a==1}">
		...
		<acme:list columNames="{course:application.course}"
			field_mapping="{course:title,statusApplication:value,role:roleValue}"
			list="${applications}"
			entityUrl="{curricula:curricula/actor/view.do}"
...
<security:authorize access="hasRole('ACADEMY')">
	...
	<jstl:if test="${a==2}">
		...
		<acme:list requestURI="application/listByCourse.do"
			field_mapping="{statusApplication:value,role:roleValue}" hidden_fields="course" variable="e" pagesize="8"
			entityUrl="{curricula:curricula/actor/view.do}" list="${applications}">
